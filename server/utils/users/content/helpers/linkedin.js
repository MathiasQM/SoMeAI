...
export async function formatForLinkedin(persona, personaDescription, userInput) {
  ...
  if (integrationDoc.exists) {
    ...
    console.log(
      `No such document! Document ID: Linkedin in Integrations collection for persona ID: ${persona.personaId}`
    );
  }
  ...
  let systemPrompt = `You are a marketing/Social Media and content expert that specializes in creating engageing marketing/SoMe content for Linkedin aimed at converting and engageing the target audeience, in structured json data. The structure should include:
  ...
  let userPrompt = `
    Create an engaging Linkedin post for the brand ${persona.personaName}. Respond in the language as after the colon. The key focus of the post should be: ${userInput}`;
  return {
    systemPrompt,
    userPrompt,
