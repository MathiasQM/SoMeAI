...
import initializeFirebaseApp from '../plugins/firebase.js'; // Adjust the path as necessary
...
export const useContentStore = defineStore('useContent', () => {
    ...
    watch(() => selectedPersona.value, (newPersona) => {
        ...
        queryUnsubscribe = onSnapshot(q, (querySnapshot) => {
            const docs = [];
    ...
    watch(() => selectedSession.value, (newSessions) => {
        console.log('Selected Session:', newSessions);
            ...
            contentUnsubscribe = onSnapshot(contentQuery, (querySnapshot) => {
    ...
    const generateContentForChannel = async (prompt) => {
        ...
        userInput.value = '';
        const requestData = { channels: selectedChannels.value, prompt, persona: selectedPersona.value, contentName: newContentName.value };
