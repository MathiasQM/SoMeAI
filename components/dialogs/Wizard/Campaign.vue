...
const contentSuggestions = ref([
  ...
  },
  {
    ...
    description: 'Dive into the world of technology, where coding meets creativity, shaping the next generation of digital wonders.',
  },
  // {
...
<template>
  <div class="w-[calc(70%)] h-[calc(80%)] p-5 flex flex-col items-center justify-center gap-5">
    <div class="w-[600px] flex flex-col items-center gap-5 border border-purple-dark rounded-lg p-5">
      <div>
        <div class="w-full flex gap-8 flex-wrap justify-start bg-creme rounded-lg pl-8 p-2 py-5 pt-10 relative">
          <label class="absolute top-2 text-center w-full left-0 text-xsmall text-darkgrey dark:text-white font-bold">Select channel</label>
          <div v-for="(channel, channelKey) in availableChannels" :key="channelKey" 
              ...
              <div class="flex flex-col items-center justify-center gap-2 w-14">
                <div class="transition-all border-2 border-[white] relative w-10 h-10 bg-purple-500 shadow-md rounded-md cursor-pointer"
                  :class="{ 'border-2 border-purple-dark scale-105': selectedChannels.includes(channelKey) }"></div>
     ...
     <div class="flex flex-wrap justify-evenly gap-2">
      <div class="group transition-all border border-creme hover:bg-creme rounded-lg p-2 flex flex-col justify-center w-[calc(50%-4px)] cursor-pointer relative" v-for="suggestion in contentSuggestions" :key="suggestion.id">
        <p class="text-psmall font-bold w-[85%]">{{ suggestion.title }}</p>
        <p class="text-xsmall leading-4 w-[85%]">{{ suggestion.description }}</p>
        <NuxtIcon name="ArrowUp" class="transition-all flex items-center justify-center absolute hover:scale-105 cursor-pointer right-0 bg-creme group-hover:bg-purple-dark w-6 h-6 text-white text-2xl pt-1 rounded-lg mr-5" />
