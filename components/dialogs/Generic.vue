...
<template>
    ...
    <Transition name="fade-bg" appear>
      <div v-show="isDialogOpen" :class="context === 'Viewer' ? 'absolute rounded-lg' : 'fixed'" class="z-20 transition-all fixed inset-0 bg-black bg-opacity-50" @click.stop="isDialogOpen = false, isDynamicDialogOpen = false"></div>
    </Transition>      <Transition name="fade" appear>
        <dialog :open="isDialogOpen || isDynamicDialogOpen" v-if="isDialogOpen || isDynamicDialogOpen" :class="context === 'Viewer' ? 'absolute rounded-lg' : 'fixed'" class="z-20 transition-all bg-white dark:bg-darkgrey text-lightgrey smd:rounded-md w-full h-full smd:h-auto top-0 smd:top-[20%] rounded-none smd:w-2/3 lg:w-1/2 lg:max-w-[800px] overflow-hidden">
          <div @click.stop="isDialogOpen = false, isDynamicDialogOpen = false" class="absolute right-5 top-5 transition-all text-lightgrey flex items-center border rounded-sm py-1 px-2 text-xsmall">
              <p class="transition-all text-lightgrey">esc</p>
          </div>

            <component :is="dynamicComponent.component" v-bind="dynamicComponentProps[type]" @update:data="handleDataUpdate" @update:sliderValue1="handleDataAge" @update:sliderValue2="handleDataAgeTwo" />
        </dialog>
      </Transition>
</template>
