...
watch(() => props.result, (newValue) => {
 ...
 <template>
    <div class="overflow-hidden bg-white rounded-lg border border-creme p-2 shadow-lg cursor-pointer">
        <div :class="isEditable ? 'overflow-y-auto pl-[10px]' : 'overflow-hidden'" class="scrollbarContainer overflow-x-hidden flex flex-col w-[400px] max-h-[72vh]">
            <div class="flex items-center p-4 gap-2 w-full relative">
                ...
                <div class="text-xsmall">
                  ...
                  <p class="bg-purple-100 text-purple-500 font-medium py-[1px] px-[2px] rounded-sm flex items-center gap-1"><NuxtIcon name="AI"/>Scheduled for: Dec. 24th at 12:00 AM</p>
                </div>
                <div class="absolute right-5 flex items-center gap-5">
                    ...
                    <NuxtIcon name="Facebook" filled class="text-3xl text-black" />
                </div>
            </div>
            <div :id="isEditable ? 'editor' : ''" :class="!isEditable ? 'overflow-hidden h-full' : ''" class="text-xsmall pre-line leading-4 !outline-none !border-none" v-html="content"></div>
            <div @click.stop="console.log('Vælg fil')" class=" w-full my-2 flex-shrink-0">
                <div class="swiper" ref="swiperContainer">
                    <div class="swiper-wrapper">
                        <div v-for="slide, i in slides" :key="`slide${i}`" :style="`background-color:${slide.bg};`" class="swiper-slide text-white">{{i}}</div>
                    </div>
