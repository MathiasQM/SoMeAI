...
watch(() => props.result, (newValue) => {
 ...
 <template>
    <div class="overflow-hidden bg-white rounded-lg border border-creme p-2 shadow-lg cursor-pointer">
        <div :class="isEditable ? 'overflow-y-auto pl-[10px]' : 'overflow-hidden'" class="custom-scrollbar overflow-x-hidden flex flex-col w-[400px] max-h-[70vh]">
            ...
            <div @click.stop="console.log('Vælg fil')" class=" w-full my-2 flex-shrink-0">
                <div class="swiper" ref="swiperContainer">
                    <div class="swiper-wrapper">
                        <div v-for="slide, i in slides" :key="`slide${i}`" :style="`background-color:${slide.bg};`" class="swiper-slide text-white">{{i}}. Vælg fil</div>
                    </div>
            ...
            <p class="mt-2"><span class="font-bold">mat.quist</span></p>
            <div :id="isEditable ? 'editor' : ''" class="mt-2 text-xsmall pre-line leading-4 !outline-none !border-none" v-html="content"></div>
        </div>
    </div>
</template>
