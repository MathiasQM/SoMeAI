...
<template>
    <div class="relative select-none">
        ...
        <div class="px-2 flex flex-col gap-2">
            <div v-for="(session, sessionIndex) in sessions" @click="openSession(sessionIndex), isMobileNavOpen = false" class="group flex justify-between items-center transition-all h-14 px-2 rounded-md hover:bg-light-100 cursor-pointer">
                <div>
                    <p class="transition-all text-small font-normal first-letter:uppercase">{{session.sessionName}}</p>
                    <p class="transition-all text-xsmall text-black dark:text-white dark:group-hover:text-purple-dark group-hover:text-purple-dark">{{formatDateTime(session.timestamp.toDate())}}</p>
                </div>
