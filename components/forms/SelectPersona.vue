<template>
  <div class="relative w-full p-4 z-30">
    <input
      ...
      autocomplete="off"
      type="text"
      class="disabled:cursor-not-allowed border border-creme rounded-md py-1 w-full pl-5 outline-none text-left dark:bg-lighterdark dark:text-white placeholder:text-lightgrey dark:placeholder:text-creme focus:outline-purple-dark cursor-pointer"
      :name="name"
      v-model="filterText"
    ...
    <Transition name="fade" appear>
      <div
        v-if="openSelect"
        class="traded-select w-52 h-full min-h-32 max-h-56 p-2 absolute z-10 bg-white dark:bg-lightdark dark:text-white dark:border-none shadow-md rounded-md border-creme border mt-1 overflow-y-scroll"
      >
        <p
          v-for="option in filteredOptions"
          :key="option"
          @click="selectOption(option)"
          class="py-2 px-1 rounded-md flex items-center gap-2 hover:bg-purple-dark/10 hover:text-purple-dark cursor-pointer"
        >
          <NuxtIcon :class="tempFilterText === option ? 'text-purple-dark' : 'text-purple-dark/0'" name="Check" />
          {{ option }}
        </p>
        <p class="absolute bottom-0 left-0 text-center p-2 w-full">Create New Persona</p>
      </div>
    </Transition>
  </div>
</template>
...
const filteredOptions = computed(() =>
  props.options.filter((option) => option.toLowerCase().includes(filterText.value.toLowerCase()))
);
