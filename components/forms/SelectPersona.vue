<template>
    <div class="relative w-full p-4 z-30">
        <input :disabled="disabled" @focus="handleInputFocus" @blur="handleInputBlur" autocomplete="off" type="text" class="disabled:cursor-not-allowed border border-creme rounded-md py-1 w-full pl-5 outline-none text-left dark:bg-lighterdark dark:text-white placeholder:text-lightgrey dark:placeholder:text-creme focus:outline-purple-dark cursor-pointer" :name="name" v-model="filterText" :placeholder="placeholder" />
        <Transition name="fade" appear>
            <div v-if="openSelect" class="traded-select w-52 h-full min-h-32 max-h-56 p-2 absolute z-10 bg-white dark:bg-lightdark dark:text-white dark:border-none shadow-md rounded-md border-creme border mt-1 overflow-y-scroll">
                <p v-for="option in filteredOptions" :key="option" @click="selectOption(option)" class="py-2 px-1 rounded-md flex items-center gap-2 hover:bg-purple-dark/10 hover:text-purple-dark cursor-pointer">
                    <NuxtIcon :class="tempFilterText === option ? 'text-purple-dark' : 'text-purple-dark/0'" name="Check"/>
                    {{ option }}
                </p>
            </div>
        </Transition>
...
onMounted(initializeValue);

watch(inputValue, (newValue) => {
    if (props.useLocalStorage) {
        saveToLocalStorage(newValue);
    }
    emit('update:modelValue', newValue); // Emit the model value update
});
